...
気に入ったＮＦＴをetherを支払って購入できる関数があるとします。etherで販売されているＮＦＴを購入するには、データの内部でvalueを設定してetherを送金します。同時に気に入ったＮＦＴのIDを指定するため、dataには引数としてIDを入れます。このトランザクションのto(recipient)は呼び出すコントラクトのアドレスになります。
